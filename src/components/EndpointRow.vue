...
<template>
  ...
  >
    ...
    <div
      v-if="
        endpointStatus.hasStarted &&
        !endpointStatus.error &&
        endpointStatus.networkStatuses.length > 0
      "
    ...
    <div class="flex items-center h-full relative z-10">
      ...
      <div
        class="px-4 py-3 w-32 text-right flex items-center justify-end"
      ></div>

      <!-- ETA column -->
      <div class="px-4 py-3 text-right w-24 flex items-center justify-end">
        <div v-if="endpointStatus.loading" class="text-gray-400">
          Loading...
        </div>
