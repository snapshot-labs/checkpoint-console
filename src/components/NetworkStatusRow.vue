...
import type { NetworkWithHistory } from '../types';
...
const { formatTimeEstimate, getBlockDifference, getSyncPercentage } = useUtils();
  ...
  >
    ...
    <div class="px-2 py-2 flex-1">
      <div class="flex items-center gap-3 pl-6">
        ...
        />
        <span class="font-normal text-base group-hover:text-white">{{ status.network.name }}</span>
        <span class="text-sm text-gray-500">({{ status.network.chainId }}, {{ status.network.indexer }})</span>
      </div>
    </div>
    ...
    <div class="px-2 py-2 text-right w-80">
      <div class="font-normal text-base text-white">
        ...
        <template v-else>
          {{ status.indexedBlock.toLocaleString() }} / {{ status.currentBlock.toLocaleString() }}
        </template>
      </div>
    </div>
    ...
    <div class="px-2 py-2 text-right w-20">
      <div v-if="isFinite(parseFloat(getSyncPercentage(status)))" class="font-normal text-base text-white">
