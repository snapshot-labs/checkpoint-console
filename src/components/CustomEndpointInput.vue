  ...
  <div class="border-b border-[#282828] h-[58px] relative overflow-visible">
    ...
    >
      <div class="flex items-center gap-3">
        <div
          class="w-8 h-8 rounded-md flex items-center justify-center bg-green-600"
        >
          <svg
    ...
    <div v-else class="px-4 py-3 flex items-center gap-3 flex-1 h-full">
      <div class="flex items-center gap-3 flex-1 relative">
        ...
        >
          <div
            v-for="(endpoint, index) in suggestedEndpoints"
            :key="endpoint"
            :class="[
              'px-3 py-2 cursor-pointer text-sm text-gray-300 hover:bg-[#2a2a2a] hover:text-white transition-colors',
              { 'bg-[#2a2a2a] text-white': index === selectedIndex }
            ]"
